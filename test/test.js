...
const ROOT_FOLDER = Config.GOOGLE_DRIVE_ROOT_FOLDER,
	...
	service_account_path = path.resolve(`${__dirname}/../config/${Config.GOOGLE_DRIVE_KEY_FILE}`),
	creds_service_user = require(service_account_path);

let temp_folder;

googleDriveInstance.useServiceAccountAuth(creds_service_user)
	.then(function (gdrivehandler) {

		return googleDriveInstance.createFolder(ROOT_FOLDER, `test_folder_${Date.now()}`);
